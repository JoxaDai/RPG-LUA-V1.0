...
-- Temps défini en Seconde (fps / 1 frame par 0.5 secondes).
function updatePlayer(dt)
  if player.canMove == true then
    ...
    -- mettre a jour la nouvelle direction (pour 4 possibilités - gauche, droite, bas, haut)
  ...
  -- Mettre a jour l'animation - demmarrage animName
  for _, animName in pairs(playerAnimations) do
    animName:update(dt)
  end
end

--- draw le joueur (sous un format de carrée 64X64 - style JUS).
function drawPlayer()
...
--- Les stats du joueurs mise par défaut.
-- Le joueur peut des le depart : <ul><li>se mouvoir</li><li>Intéragir avec des objets</li><li>ouvrir son inventaire</li><li>
-- ouvrir le menu (sous autre fichier menu.lua</li><li>visible</li></ul>
