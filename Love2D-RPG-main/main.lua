...
-- Controles et inputs (mouvements ^^).
controls = { up = { keys = {"w", "up"}, held = 0}, down = { keys = {"s", "down"}, held = 0},
             ...
             attack = { keys = {"x", "z", "q", "space"}, held = 0 --[[ Attack system TODO ]]}, interact = { keys = {"e", "return"}, held = 0} }
...
-- chargement niveau principal
currentlvl = LoadLevel("levels/main")

--- Execution au démarrage 
function love.load()
  min_dt = 1/60 -- FPS frame par second defaut
  next_time = love.timer.getTime()

  -- mettre joueur sur la carte et le suivre de vue aérienne par la caméra
...
--- Execute les frames selon le temps 
...
--- Detection de la touche pressé du clavier
function love.keypressed(keypressed, scancode, isrepeat)
  -- Tdebug mod si matient de la touche \ ou annuler
